...
### 2.2 Scope（归属域）——通用隔离模型

Scope 是**任意字符串标识**，不预设任何具体 Agent 类型：

```
scope 示例（纯约定，服务不感知语义）：
  "agent:pi-coding"      # 某编码 Agent
  "user:zhangyunfei"     # 某用户
  "team:infra"           # 某团队共享域
  "project:billing"      # 某项目域
```

- 服务**不解析** scope 语义，只做字符串匹配 + 权限校验
- 读写权限由配对时授予的 Scope 权限决定（见 §7）
...
| 实体 | 查询实体 → 关联条目 | 实体级精确 |
| 关系（可选） | 图遍历一跳邻居（M-FLOW 风格） | 关联推理 |

### 6.2 多因子评分（v2.0 优化）

```
score = w1×text_sim + w2×recency + w3×gravity + w4×freq + w5×scope_boost
...
权重默认：w1=0.4, w2=0.3, w3=0.15, w4=0.1, w5=0.05（全部可配置）
```
...
1. **LLM 提取是否为默认开启**？（开=质量高但依赖 LLM；关=纯本地零依赖）建议：默认开，可配置关
2. **对象存储选型**：阿里云 OSS（国内）还是 S3 兼容（通用）？建议：实现 S3 兼容协议，OSS/MinIO 都通
3. **配对确认方式**：CLI 命令 / Web 管理页 / API 三选一？建议：CLI 先行，Web 后补
4. **Scope 权限粒度**：是否需要"Zone 级"权限（某 key 只能读写指定 Zone）？建议：v2.0 先做 scope 级，Zone 级预留
5. **检索是否需要图遍历**（M-FLOW 关系检索）：v2.0 默认只做语义+词法+实体，图遍历作为可插拔策略？建议：先不做，接口预留

---

*方案 v2.0 结束。确认后进入代码设计（P1）。*
